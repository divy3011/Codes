...
int main(){
    cout<<"\n";
    set<char> non_ter{'S','T'};
    set<char> terminal{'x','y'};
    ter['S'-65].push_back("xSy");
    ter['S'-65].push_back("T");
    ter['S'-65].push_back("xy");
    ter['T'-65].push_back("xTy");
    ter['T'-65].push_back("xy");
	...
	cin>>noskipws>>s;
    if(s==""){
        cout<<"\tEmpty String is accepted by the Grammar"<<endl;
        return 0;
    }
	queue<string> q;
	for(i=0;i<ter[18].size();i++){
